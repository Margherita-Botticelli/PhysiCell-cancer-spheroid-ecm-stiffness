...
def plots_mot_vs_degr_delaunay(data, simulation_name, save_folder, title=True):
    ...
    for simulation in simulations:
        ...
        # print(f'{df_sim=}', flush=True)

        delaunay_distance = df_sim['delaunay_distance'].to_numpy()
        # print(f'{delaunay_distance=}', flush=True)
    ...
    # ax = seaborn.heatmap(df,cmap=cmap,vmin=15, vmax=35,annot = annot_arr, fmt="s", cbar_kws={'label': 'Delaunay mean distance'})
    ax = seaborn.heatmap(df,cmap=cmap,vmin=15, vmax=35, cbar_kws={'label': 'Delaunay mean distance'})
    # ax = seaborn.heatmap(df,cmap=cmap,vmin=0, vmax=7,annot=True,fmt='.2f', cbar_kws={'label': 'Growth relative to t$_0$'})

    ax.figure.axes[-1].yaxis.set_label_position('left')

    ###  Set axis labels
    plt.ylabel("Degradation rate")
    plt.xlabel("Max motility speed")
    ...
    plt.savefig(save_folder + f'plots/mot_vs_degr_delaunay_rib{ribose}_{simulation_name}_t{int(max(t)/60)}.png', bbox_inches = "tight")
    plt.close()


def plots_mot_vs_degr_spheroid_growth(data, simulation_name, save_folder, title=True):
    ...
    # ax = seaborn.heatmap(df,cmap=cmap,vmin=0, vmax=8,annot = annot_arr, fmt="s", cbar_kws={'label': 'Growth relative to t$_0$'})
    ...
    plt.savefig(save_folder + f'plots/mot_vs_degr_spheroid_growth_rib{ribose}_{simulation_name}_t{int(max(t)/60)}.png', bbox_inches = "tight")
    plt.close()


def plots_alpha_vs_beta_spheroid_growth(data, simulation_name, save_folder, title=True):
    ...
    annot_df = annot_df.fillna('NaN')
    # print(annot_df,flush=True)
    for alp,bet,area_mean,area_std in zip(alpha,beta,spheroid_area_ratio_mean,spheroid_area_ratio_std):
        df[alp][bet] = area_mean

        annot_mean = "%.2f" % area_mean
        annot_std = "%.2f" % area_std
        annot = annot_mean + '\n' + u'\u00B1' + annot_std
        annot_df[alp][bet] = annot
    ...
    plt.savefig(save_folder + f'plots/alp_vs_bet_spheroid_growth_rib{ribose}_{simulation_name}_t{int(max(t)/60)}.png', bbox_inches = "tight")
    plt.close()
...
def plots_adh_vs_rep_spheroid_growth(data, simulation_name, save_folder):
    ...
    ax = seaborn.heatmap(df,cmap=cmap,vmin=0, vmax=20,annot = annot_arr, fmt="s", cbar_kws={'label': 'Growth relative to t$_0$'})
    # ax = seaborn.heatmap(df,cmap=cmap,vmin=0, vmax=7,annot=True,fmt='.2f', cbar_kws={'label': 'Growth relative to t$_0$'})
...
def plots_adh_vs_rep_clusters(data, simulation_name, save_folder):
    ...
    ax = seaborn.heatmap(df,cmap=cmap,vmin=0, vmax=100,annot = annot_arr, fmt="s", cbar_kws={'label': 'Growth relative to t$_0$'})
    # ax = seaborn.heatmap(df,cmap=cmap,vmin=0, vmax=7,annot=True,fmt='.2f', cbar_kws={'label': 'Growth relative to t$_0$'})
