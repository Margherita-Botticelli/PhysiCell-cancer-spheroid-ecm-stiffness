...
ifeq ($(OS),Windows_NT)
	...
	ifeq ($(UNAME_S),Darwin)
		...
		ifeq ($(lastword $(var)),arm64)
		  CFLAGS := -march=$(ARCH) -O3 -fomit-frame-pointer -fopenmp -m64 -std=c++11 -Werror
		endif
	endif
endif
...
PhysiCell_custom_module_OBJECTS := custom.o extracellular_matrix.o
...
save: 
	...
	mkdir -p ./user_projects/$(PROJ)/output/output
	...
	cp ./output/* ./user_projects/$(PROJ)/output/output
